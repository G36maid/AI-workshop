...
> **Provenance**：此心智模型的直接出處 = ThePrimeagen Standup podcast 中 **Adam 在 [04:32] 定義「Agent = LLM + Tools + Loops」**（見 `references.md` §7.4）——講者從此吸收內化。
...
### 延伸：如何讓 agent 持續下去（持續性機制的兩派）
INSIGHT-3 說 agent = loop。但 agent 有個壞習慣：**會偷懶提早結束**（幻覺完成、喊 done 其實沒做）。所以生態發明了「持續性機制」強迫它做完，有兩個學派：
- **Todo-driven（Sisyphus / omo 的 Todo Enforcer）**：看 agent 自建的 todo list；未完就 hook 注入繼續。尊重 agent 的計畫，session 連續。
- **Prompt-reinjection（Ralph Loop）**：每次 agent 結束就把同一 prompt 重注入；檔案系統 + git 當記憶體，每輪可 fresh context。

→ 兩派都是「**把 INSIGHT-3 的 loop 強化成不會中途停下的版本**」，差別在「誰決定還沒完」（todo vs 固定 prompt）與「記憶放哪」（session vs filesystem）。完整生態見 `references.md` §9。
...
### 實證：omo ↔ Anthropic 大戰（INSIGHT-4 不是哲學，是戰場）
2026 上半年，**omo 的多模型編排（= INSIGHT-4 的實作）literally 觸發了 Anthropic 對 OpenCode 的封殺**：
- omo 把 Claude Max 訂閱的 OAuth token 接進 OpenCode 做 multi-agent，token 消耗暴增 → Anthropic 注意 → 2026-01-09 伺服器端封鎖 OAuth → 2026-03-19 發律師函移除所有 Claude 整合。
- Dax 公開：「Anthropic blocked OpenCode because of us [omo].」
- **omo 被封殺的瞬間直接切到 Hephaestus（GPT-native）+ GLM + Kimi**——被鎖死的使用者會斷炊，omo 使用者只是換條線。**這就是 INSIGHT-4 的價值兌現**。
- 完整時間軸與來源見 `references.md` §10。

→ 對學生同時是**警告**：用 OAuth 訂閱接第三方工具有帳號 ban 風險（終身封、申訴爛）；要用就用 API key。
→ **更深一層**：這不是 Anthropic 的單一事件，而是跨標準的封閉模式（AGENTS.md / ACP / SKILLS 位置 / opencode 法律行動 / claude -p 黑箱）——完整清單見 `references.md` §11。講者「no vendor lock-in」的堅持有道德維度，不只是技術偏好。

### ⚠️ 重要 nuance：split 不是免費午餐（refines 本條）
INSIGHT-4 的「貴規劃 + 便宜實作」**有隱藏成本**，2026 上半年多方實測指出：
- **複雜任務（async / 並行 / 外部依賴）上，便宜 executor 即使有完美 plan 也無法 close the gap**——「plan closes the WHAT gap, leaves the HOW gap open」（CodeMiner42）。
- plan 不攜帶事實 API 知識 → executor 幻覺 API（AkitaOnRails 實例：Qwen 把 `RubyLLM.chat` 寫成不存在的 method）。
- 每次 plan→execute→fail→re-plan 的協調成本，可能超過單一大模型從頭做完。
- **結論**：「You pay for the expensive model so you don't have to babysit, not for the tokens.」——split 在簡單/範圍清楚任務上省很大，在複雜任務上 math 會翻轉。

→ 教學判據：**這任務的 HOW（實作 / API 知識 / 協調）跟 WHAT（規劃）一樣重要嗎？若 HOW 也難，用單一強模型反而省**。完整實測與來源見 `references.md` §12。
...
## INSIGHT-9：Insight 來自初級來源，不來自搬運層 — 改變資訊飲食 = 改變洞察深度

> **來源**：講者陳述（2026-07-01），見 `personal-info.md` §5.12

### 核心命題
講者觀察：**繁中 AI 社群大量內容只是搬運（repost）或工具用法介紹，缺乏真正的 insight**。對比之下，講者的 insight 來自直接消費**初級來源**——英文圈的硬核創作者與開發者討論（詳見 `personal-info.md` §5.12）。

→ 這些都是**生產 insight 的源頭**，而繁中搬運文是**二手再生層**——後者註定落後、稀釋、失真。

### 跟 INSIGHT-1 的漂亮類比（亮點）
**LLM 是 I/O 黑盒子，人也是。** 你餵大腦什麼資訊飲食，就長出什麼判斷。「Garbage in garbage out」對人對模型同構——改善「人的 input」跟改善「LLM 的 input」是同一件事。講者有 insight，本質上就是因為他做對了**人類版的 context engineering**。

### 這條洞察拆穿了什麼
- **拆穿「我吸收很多 AI 資訊所以懂 AI」**：若來源是搬運層，吸收再多也只是工具用法，長不出 insight。
- **拆穿「繁中 AI 社群 ≈ 英文 AI 社群」**：兩者間有結構性落差——一個生產，一個搬運。要 insight 必須跨過語言牆。
- **拆穿「我得自己從零悟出 insight」**：不必。insight 是可以被「換資訊飲食」培養的——去初級來源吸收別人已生產的 insight，再內化。

### 對 workshop 的啟示（**雙重價值**）
- **定位錨點**：這個 workshop 的核心價值主張，正是**提供繁中社群缺的那層 insight**——不是教工具用法（那是搬運層在做的事），而是教「為什麼這樣用」的心智模型（INSIGHT-1~8）。這是講者相對於繁中 AI 內容圈的獨特資產。
- **可教 meta-skill**：告訴學生「**想變得有 insight，先換資訊飲食**」——給出講者的來源清單當起點。這是少數能直接「教」的 meta-skill。
- **誠實 framing**：把 INSIGHT-1~8 定位為「我們直接把英文圈的 insight 用中文講給你聽，省你爬源頭的時間」——對學生誠實且實用。
